...
function mostrarRecados(resposta) {
    ...
    for (const valor of dados) {
        ...
        <tr>
            ...
            <td class="col-3" id="des${valor.id}">${valor.description}</td>
            <td class="col-6" id="det${valor.id}">${valor.details}</td>
...
function criarRecado() {
    ...
    axios.post(`${api}/note/${userLogged}`, {
        description: descri, details: detalhes
    }).then((resposta) => {
...
function editarRecados(rid) {
    const descri = document.getElementById('inputDescri').value;
    const detalhes = document.getElementById('inputDetalha').value;
    axios.put(`${api}/note/${rid}`, {
        description: descri, details: detalhes, userID: userLogged,
    }).then((resposta) => {
...
// Apagar recado ok
function apagar(rid) {    
    axios.delete(`${api}/note/${rid}`).then((resposta) => {
